...
public class TestTemplates {
	...
	private Logic.ProgramData printProgram(String s) throws IOException {
		System.out.println(s);
		Logic.ProgramData data = Logic.readProgram(Encoder.decodeFromBase64(s), null);
		for (int i : data.intBlock) {
			System.out.println(i);
		}
		for (byte[] b : data.byteBlock) {
			if (b.length == 32) {
				Address a = new Address(b);
				System.out.println(a.toString() + " / " + Encoder.encodeToBase64(b));
			} else {
				System.out.println(Encoder.encodeToBase64(b));
			}
		}
		return data;
	}
	...
	public void testDynamicFee() throws Exception {
		...
		*/
		...
		for (SignedTransaction stxn : stxns) {
		    byte[] msgpack = Encoder.encodeToMsgPack(stxn);
			stxStr += Encoder.encodeToBase64(msgpack);
			Map m = Encoder.decodeFromMsgPack(msgpack, Map.class);
		...
		Map mGold = Encoder.decodeFromMsgPack(goldenStxns, Map.class);
		Map mActual = Encoder.decodeFromMsgPack(stxStr, Map.class);
